...
export class Namespace {
  ...
  serialize(): FrozenByteBuffer {
    ...
    buf.putVI(ControlMessageType.Namespace)
    const payload = new ByteBuffer()
    payload.putTuple(this.trackNamespaceSuffix)
    const payloadBytes = payload.toUint8Array()
    if (payloadBytes.length > 0xffff) {
      throw new LengthExceedsMaxError('Namespace::serialize(payloadBytes.length)', 0xffff, payloadBytes.length)
    }
  ...
  static parsePayload(buf: BaseByteBuffer): Namespace {
    ...
    return new Namespace(trackNamespaceSuffix)
  }
}
...
if (import.meta.vitest) {
  ...
  describe('Namespace', () => {
    test('roundtrip', () => {
      ...
      const msg = new Namespace(suffix)
      const frozen = msg.serialize()
      const msgType = frozen.getVI()
      expect(msgType).toBe(BigInt(ControlMessageType.Namespace))
      const msgLength = frozen.getU16()
      expect(msgLength).toBe(frozen.remaining)
      const deserialized = Namespace.parsePayload(frozen)
